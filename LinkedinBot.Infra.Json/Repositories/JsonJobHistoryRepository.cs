...
namespace LinkedinBot.Infra.Json.Repositories;

public class JsonJobHistoryRepository : IJobHistoryAppService
{
    ...
    public JsonJobHistoryRepository(IOptions<DataConnectionSettings> settings, ILogger<JsonJobHistoryRepository> logger)
    {
        var filePath = string.IsNullOrWhiteSpace(settings.Value.FilePath)
            ? "./job-history.json"
